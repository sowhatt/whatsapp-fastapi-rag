(() => {
  ...
  style.href = '/auth/express-sale.css?v=1';
  ...
  function add(productId) {
    ...
    cart.set(product.id, { product, quantity: nextQty }); renderExpressSale();
  }
  function change(productId, delta) {
    const line = cart.get(Number(productId)); if (!line) return;
  ...
  async function checkout() {
    if (!cart.size) return;
    const customerValue = $('expressCustomer').value, customerId = customerValue ? Number(customerValue) : null, channel = $('expressChannel').value, amount = total(); let paidAmount = amount;
    if (channel === 'credit') { if (!customerId) return toast('Choisissez un client pour une vente à crédit'); paidAmount = 0; }
    else if (channel === 'partial') { if (!customerId) return toast('Choisissez un client pour un paiement partiel'); const raw = window.prompt(`Montant encaissé sur ${money(amount)}`, '0'); if (raw === null) return; paidAmount = Number(raw); if (!Number.isFinite(paidAmount) || paidAmount < 0 || paidAmount > amount) return toast('Montant payé invalide'); }
    ...
    try {
      ...
      cart.clear(); query = ''; $('expressSearch').value = ''; await refresh(); syncExpressCustomers(); renderExpressSale(); toast('Vente express enregistrée');
    } catch (error) { toast(error.message); renderExpressSale(); }
  }
  function syncExpressCustomers() {
    const select = $('expressCustomer'); if (!select) return; const selected = select.value;
  ...
  document.addEventListener('click', (event) => { const addBtn = event.target.closest('[data-add-product]'); if (addBtn) return add(addBtn.dataset.addProduct); const plus = event.target.closest('[data-cart-plus]'); if (plus) return change(plus.dataset.cartPlus, 1); const minus = event.target.closest('[data-cart-minus]'); if (minus) return change(minus.dataset.cartMinus, -1); });
  ...
  const originalRender = render; render = function () { originalRender(); syncExpressCustomers(); renderExpressSale(); };
  syncExpressCustomers(); renderExpressSale();
})();